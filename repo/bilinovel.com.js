...
// @icon        https://raw.githubusercontent.com/appdevelpo/repo/refs/heads/miru_alpha/icon/bilinovel.com.ico
// ==/MiruExtension==
export default class extends Extension {
  ...
  filter(element) {
    if (element.content.startsWith("<img")) {
      ...
      return `${match.length ? "" + match[match.length - 1][1] : ""}`;
    }
  ...
  async latest(page, filter) {
    const res = await this.$api(
      `/top/${filter?.ranks[0] ?? "lastupdate"}/${page}.html`,
    );
    ...
    if (total) {
      ...
      const tasks = list.map(async (e) => {
        ...
        if (author.indexOf("hot") > 0) {
          author = author.match(/class="hot">(.*)<\/span>/)[1].trim() +
            author.split("</span>")[1];
        }
        const url = `${e.content.match(/href="(.*?)"/)[1]}|${title}|${cover}`;
